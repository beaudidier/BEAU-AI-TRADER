...
class SwingTradingStrategy(BaseStrategy):
    """The frozen Regime-Gated Pullback strategy used for forward validation."""
...
swing_trading_strategy = SwingTradingStrategy(
    id="swing_trading",
    name="Swing Trading",
    status=StrategyStatus.FORWARD_VALIDATION,
    asset_classes=("US stocks",),
    supported_timeframes=("1D",),
    required_data=(
        "At least 200 completed daily OHLCV candles",
        "SPY benchmark daily OHLCV",
        "EMA20",
        "ATR",
        "20-session swing low",
        "Institutional market-regime score",
    ),
    scanner_rules=(
        "Long-only US stocks",
        "Market-regime score must be at least 65 at signal time",
        "Reject invalid ATR, stop, risk, or market data",
    ),
    entry_rules=(
        "Place an EMA20 pullback limit for three completed sessions",
        "Apply 5 basis points of entry slippage",
    ),
    stop_rules=(
        "Stop is 1.5 ATR below the signal-time 20-session swing low",
        "Keep the original stop after target 1",
        "Use stop-first handling when stop and target trade in the same candle",
    ),
    target_rules=(
        "Target 1 is 2R and closes 50% of the position",
        "Target 2 is 4R and closes the remainder",
    ),
    holding_period="Maximum 30 completed daily candles after entry",
    risk_limits=(
        "Reject trades whose per-share risk exceeds 5% of expected entry",
        "Forward-validation paper trading only",
        "No overlapping rule changes from the frozen v1.0.0 strategy",
    ),
)
