...
function paperTradeBlockReasons(plan: TradePlan, stock: Stock): string[] {
  ...
  if (!plan.trade_allowed) reasons.push(...plan.rejection_reasons);
...
function StockDetailPanel({ stock, onClose }: StockDetailPanelProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    setTradePlan(null); setTradePlanError(null); setAnalysis(null); setAnalysisError(null); setConfirmingTrade(false); setPaperTradeError(null); setOpenedTradeId(null); setPortfolio(null);
    ...
    void userApi.paperPortfolio().then((result) => { if (!cancelled) setPortfolio(result as PaperPortfolio); }).catch(() => { if (!cancelled) setPaperTradeError("Paper risk capacity is temporarily unavailable. Refresh before opening a trade."); });
  ...
  async function confirmPaperTrade() {
    ...
    try {
      ...
      setOpenedTradeId(created.id); setPortfolio(await userApi.paperPortfolio() as PaperPortfolio); setConfirmingTrade(false);
