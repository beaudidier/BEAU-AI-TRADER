...
import { createScanJob, getDailyBriefing, getScanJob, getScanJobResults, scanMarket } from "../services/api";
import type { DailyBriefing, ScanJob, Stock } from "../types/stock";
...
const universeOptions = [{ value: "stocks:demo", label: "Demo" }, { value: "stocks:sp500", label: "S&P 500" }, { value: "stocks:nasdaq100", label: "Nasdaq 100" }, { value: "stocks:dow30", label: "Dow 30" }, { value: "stocks:all_us", label: "All US Stocks" }, { value: "crypto:top50", label: "Crypto Top 50" }, { value: "crypto:top100", label: "Crypto Top 100" }];
...
function Dashboard({ onOpenChart, onNavigate, searchTerm, onSearchChange }: DashboardProps) {
  ...
  const startSelectedScan = useCallback(async () => {
    ...
    if (market === "stocks" && selectedUniverse === "demo") { setActiveScan(null); await refreshScanner(); return; }
    setScannerLoading(true); setScannerError(null); setStocks([]);
    try {
      const job = await createScanJob(market, selectedUniverse);
      setActiveScan(job);
      if (job.status === "completed") {
        const result = await getScanJobResults(job.job_id);
        setStocks(result.results); setScannerLoading(false);
      }
    }
    catch { setScannerError("This market scan could not be started. Please try again."); setScannerLoading(false); }
  }, [refreshScanner, universe]);

  useEffect(() => {
    if (!activeScan || activeScan.status === "completed") return undefined;
    const timer = window.setTimeout(() => {
      void getScanJob(activeScan.job_id).then(async (job) => {
        setActiveScan(job);
        if (job.status === "completed") {
          const result = await getScanJobResults(job.job_id);
          setStocks(result.results); setScannerLoading(false);
        }
      }).catch(() => { setScannerError("The scan stopped before results were available. Please try again."); setScannerLoading(false); });
    }, 700);
